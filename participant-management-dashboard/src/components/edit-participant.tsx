...
export default function EditParticipant({ participant, updated }: { participant: Participant, updated: () => void }) {
  ...
  async function save() {
    try {
      await axios.patch('http://localhost:5000/participants', {
        id: participant.id,
        name,
        email,
        avatar
  ...
  return (
    <div>
      <a onClick={openModal} className="font-medium text-blue-600 dark:text-blue-500 hover:underline cursor-pointer">Edit</a>
      <Modal
        isOpen={modalIsOpen}
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-6 gap-6">
                <div className="col-span-6 sm:col-span-3">
                  ...
                  <input value={participant.id} disabled type="text" name="id" id="id" className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="1297" required />
                </div>
                <div className="col-span-6 sm:col-span-3">
