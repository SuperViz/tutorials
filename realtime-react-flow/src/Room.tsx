import { Comments, MousePointers, Realtime, useComments, useHTMLPin, useMouse, useRealtime, WhoIsOnline } from "@superviz/react-sdk";
import { useCallback, useEffect, MouseEvent, useRef } from "react";
import ReactFlow, { useNodesState, Controls, Background, ConnectionLineType, addEdge, useEdgesState, ConnectionMode, Connection, useViewport, Node } from 'reactflow'
...
export default function Room({ participantId }: Props) {
  ...
  useEffect(() => {
		...
		subscribe('node-drag', ({ data, participantId: senderId }: { data: { node: Node }, participantId: string }) => {
			if (senderId === participantId) return;
